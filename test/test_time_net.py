...
def main():
    ...
    b = torch.from_numpy(b).to(torch.complex64)
    sigma = torch.from_numpy(sigma).to(torch.float32)

    model = PhiEstADMMNet(num_layers=5, M=Nb, N=Nd, L=L)
    checkpoint = torch.load('../checkpoints/20251227-15-10/best_model.pth', weights_only=False)
...
if __name__ == '__main__':
    ...
    # 保存到文件
    np.savetxt('time_net_5.txt', cur_time)